...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [asset] = await db.insert(mediaAssets).values({ ownerId: user.id, originalName, storageKey: key, thumbnailKey: thumbKey, mimeType: mime, sizeBytes, ...meta }).returning();
    await db.update(projects).set({ assetId: asset.id, updatedAt: new Date() }).where(eq(projects.id, id));
