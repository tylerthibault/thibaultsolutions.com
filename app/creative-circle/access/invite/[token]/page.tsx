...
export default async function CreativeCircleAccessInvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const sections = [invite.labAccess ? "Video Lab" : null, invite.feedbackAccess ? "Feedback Lab" : null]
    .filter(Boolean).join(" + ");
  const identifier = invite.username ? `@${invite.username}` : invite.email;

  return <main className="login-wrap"><section className="login-card">
    <span className="micro" style={{ color: "var(--lime)" }}>PRIVATE INVITE / CREATIVE CIRCLE</span>
    <h1>YOU’RE<br/>INVITED.</h1>
    <p className="muted">You were invited as <b style={{ color: "var(--ink)" }}>{identifier}</b>.</p>
    ...
    <AccessInviteAcceptForm token={token} identifier={identifier} signInMode={invite.username ? "username" : "email"} signedIn={Boolean(session?.user)}/>
  </section></main>;
}
